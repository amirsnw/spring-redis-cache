...
public class UnifiedRedisCache implements Cache {
    ...
    private Logger log = LoggerFactory.getLogger(UnifiedRedisCache.class);
     ...
     */
    public UnifiedRedisCache(String name, byte[] prefix, RedisOperations<?, ?> redisOperations,
                             long expiration,
                             String applicationVersion) {

        Assert.hasText(name, "CacheName must not be null or empty!");
        Assert.notNull(redisOperations.getValueSerializer(),
                "The Redis template must have a value serializer when using this cache.");
        ...
        this.currentVersionBytes = versionSerializer.serialize(applicationVersion);
    }
    ...
    public ValueWrapper get(final Object key) {
        ...
        } catch (RedisConnectionFailureException | TooManyClusterRedirectionsException |
    ...
    public ValueWrapper putIfAbsent(Object key, final Object value) {
        //Really don't need this semantic for our use case, we just call the standard put and return the value we are setting.
        put(key, value);
    ...
    static class RedisCacheCleanByPrefixCallback implements RedisCallback<Void> {
        ...
        public Void doInRedis(RedisConnection connection) {
            ...
            if (isClusterConnection(connection)) {
                ...
                Set<byte[]> keys = connection.keyCommands().keys(prefixToUse);
                if (keys != null && !keys.isEmpty()) {
                    connection.keyCommands().del(keys.toArray(new byte[keys.size()][]));
                }
            } else {
                assert REMOVE_KEYS_BY_PATTERN_LUA != null;
                connection.scriptingCommands().eval(REMOVE_KEYS_BY_PATTERN_LUA, ReturnType.INTEGER, 0, prefixToUse);
            }
